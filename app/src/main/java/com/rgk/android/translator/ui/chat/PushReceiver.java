...
import com.rgk.android.translator.mpush.sdk.MPushService;
...
public class PushReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        if (MPushService.ACTION_MESSAGE_RECEIVED.equals(intent.getAction())) {
            ...
            Log.i("jingyi", "BroadcastReceiver message="+message);

            Toast.makeText(context.getApplicationContext(), "收到新的通知：" + message, Toast.LENGTH_SHORT).show();

            if (messageId > 0) MPush.I.ack(messageId);

            if (TextUtils.isEmpty(message)) return;

            MessageResponse ndo = fromJson(message);
            Log.i("jingyi", "ndo="+ndo.toString());
